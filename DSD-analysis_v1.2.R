...
embryo2 <- cbind(two.d.2, as.character(ID), as.character(devTemp))
embryo4 <- cbind(two.d.4, as.character(ID), as.character(devTemp))

write.csv(x = embryo2, file = "embryo2.csv", sep = ",", col.names = TRUE, row.names = FALSE)
write.csv(x = embryo4, file = "embryo4.csv", sep = ",", col.names = TRUE, row.names = FALSE)
write.csv(x = cells2, file = "cells2.csv", sep = ",", col.names = TRUE, row.names = FALSE)
write.csv(x = cells, file = "cells4.csv", sep = ",", col.names = TRUE, row.names = FALSE)


### Plot specimens to check that everything's working properly ------------------
plotAllSpecimens(gpa.2$coords, mean = T, links = cell.link.2, plot.param = list(pt.bg = as.factor(paste(ID, devTemp, sep="-")), pt.cex = 0.8, mean.bg = "gray", mean.cex = 1.2, link.col = "gray", link.lwd = 2, link.lty = 1))
plotAllSpecimens(gpa.4$coords, links = cell.link.4, mean = TRUE, plot.param = list(pt.bg = as.factor(paste(ID, devTemp, sep="-")), pt.cex = 0.8, mean.bg = "gray", mean.cex = 1.2, link.col = "gray", link.lwd = 2, link.lty = 1))
...
gpa20 <- arrayspecs(subset(two.d.4, devTemp == "20C"), 18, 2)
plotTangentSpace(gpa20, groups = col.gp, legend = T)
...
plot(pcCell.4[,1], pcCell.4[,2], col = as.factor(col.gp), bg = as.factor(col.gp), pch=21)
pcCluster <- kmeans(pcCell[,1:2], 4, 99, 20)
points(pcCell.4[,1], pcCell.4[,2], col = as.factor(pcCluster$cluster), pch=18, cex = 0.75 )


Y <- two.d.array(gpa.4$coords)
fit.null = lm(Y ~ 1)
fit.ID = update(fit.null, . ~ . + ID)

  man.null <- manova(Y ~ 1)
  man.id <- manova(Y ~ 1 + ID)
  summary.manova(man.id, tol = 0)
...
plotRefToTarget(ref.4, lsmeans.4[,,2], links = cell.link.4, mag = 3, method = "TPS", gridPars = GP3)  
...
plotRefToTarget(ref.4, lsmeans.4[,,1], links = cell.link.4, mag = 3, method = "TPS", gridPars = GP3)
...
procD.lm(gpa.2$coords ~ ID * devTemp, iter=999, RRPP=T)
procD.lm(gpa.4$coords ~ ID * devTemp, iter=999, RRPP=T)
